import React, { Component } from 'react';
import {Link } from 'react-router-dom';

var image = "data:image/png;base64,"

export default class Header extends Component {
    // constructor() {
    //     super();

        
    // };
    render() {
        console.log(window.location.hash)
        var headerStuff = '';
        var location = window.location.hash;
        if (location === "#/")  {
            
            headerStuff = <div className="headerContent">
                            <Link to="/">
                                <img src={image} alt="shelfie-logo" className="shelfieLogo" />
                                <span className="shelfieSpan">SHELFIE</span>
                            </Link>
                            </div>
                
            } else if (location.split("/")[1] === "bins") {
                console.log('hdhfhjfghj')
            headerStuff = <div className="binsContent">
                                
                                <div className="logoContainer">
                                    <Link to="/">
                                    <img src={image} alt="shelfie-logo" className="binsLogo" />
                                    </Link>
                                </div>
                                <div className="binsSpanContainer">
                                    <span className="binsSpan">
                                        {`Shelf ${location.split("/")[2]}`}
                                    </span>
                                </div>
                                
                                
                            </div>
            } 
        
        return (
            <div className="header">
                {headerStuff}
            </div>
        )
    }
}